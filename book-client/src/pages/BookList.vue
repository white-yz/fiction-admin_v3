...
<template>
    <div class="song-list">
        <ul class="song-list-header">
            <li v-for="(item,index) in bookStyle" :key="index" @click="handleChangeView(item.name)"
...
<script>
import ContentList from '../components/ContentList';//单本书
...
export default {
    ...
    methods:{
        ...
        goBookListOfStyle(style){
            getBookListOfLikeStyle(style, 0)
                .then(res =>{
                        this.currentPage = 1;
                        this.albumDatas = res;
                    })
